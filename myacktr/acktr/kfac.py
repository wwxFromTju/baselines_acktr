...
import numpy as np
import re
from myacktr.acktr.kfac_utils import *
        ...
        def searchFactors(gradient, graph):
            ...
            if 'AddN' in bpropOp_name:
                ...
                if fpropOp.op_def.name in KFAC_OPS:
                    # Known OPs
                    ###
                    print('in ops:', [i for i in bpropOp.inputs if 'gradientsSampled' in i.name])
                    bTensor = [
                    ...
                    print('bInputsList', bInputsList)
                    if len(bInputsList) > 0:
                        bTensor = bInputsList[0]
                        bTensorShape = fpropOp.outputs[0].get_shape()
                        print(bTensorShape, bTensor.get_shape(), bTensor, tf.shape(bTensor), bTensor.get_shape())
                        try:
