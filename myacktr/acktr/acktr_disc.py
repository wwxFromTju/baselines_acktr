...
from myacktr.common import set_global_seeds, explained_variance

from myacktr.a2c.a2c import Runner
from myacktr.a2c.utils import discount_with_dones
from myacktr.a2c.utils import Scheduler, find_trainable_variables
from myacktr.a2c.utils import cat_entropy, mse
from myacktr.acktr import kfac
...
def learn(policy, env, seed, total_timesteps=int(40e6), gamma=0.99, log_interval=1, nprocs=32, nsteps=20,
    ...
    reward_mean = tf.Variable(0, tf.float32)
    ...
    for update in range(1, total_timesteps//nbatch+1):
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.record_tabular('value', np.mean(values))
            logger.dump_tabular()

            summary_writer.add_summary(sess.run(reward_mean_op, {reward_mean: np.mean(rewards)}), update)
